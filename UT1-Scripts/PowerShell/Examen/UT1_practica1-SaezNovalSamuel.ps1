param(
  ...
  [string]$SessionCode = "UT1_P1_SSN"
  #[string]$outputPath = "D:\Usuarios\ASR203\Downloads\Practica_PS",
  #[string]$LogPath = "D:\Usuarios\ASR203\Downloads\Practica_PS\Logs",
  #[string]$SessionCode = "UT1_P1_SSN"    
)
...
$csv = "$outputPath\$env:computername-$SessionCode.csv"
...
$ErrorLogPath = "$logpath\$SessionCode.log"
...
$checkInstalledApps = 0
  ...
  #Creacion de la carpeta Pracicas_PS

  if (-not (Test-Path $outputPath)) {
    try{
        Write-Host "Creando la carpeta Pracicas_PS en la ruta especificada."
        New-Item -ItemType Directory -Path $outputPath
    }
    catch{
        Write-Host "No se ha podido crear la carpeta Pracicas_PS en la ruta especificada."
        New-Item -ItemType Directory -Path [environment]::GetFolderPath("MyDocuments") + "\Practica_PS\Logs"
    }
}
  ...
  try {
        Write-Host "Creando la carpeta Logs en la ruta especificada."
        New-Item -ItemType Directory -Path $LogPath
  }
    catch {
        Write-Host "No se ha podido crear la carpeta Logs en la ruta especificada."
        New-Item -ItemType Directory -Path [environment]::GetFolderPath("MyDocuments") + "\Practica_PS\Logs"
    }
}
...
{
  Write-Host "Creando archivo de log de errores..."
  try {
    New-Item -ItemType "file" -Path $ErrorLogPath
    icacls $ErrorLogPath /grant Everyone:F
  }
  catch {
    Write-Host "No se ha podido crear el archivo de log de errores en la ruta especificada."
    $ErrorLogPath = [environment]::GetFolderPath("MyDocuments") + "\Practica_PS\Logs\$SessionCode.log"
    New-Item -ItemType "file" -Path $ErrorLogPath
    icacls $ErrorLogPath /grant Everyone:F
  }
}

# Prueba de escritura en la ruta especificada

  function Test-WriteAccess {
    try {
        write-host "Probando permisos de escritura en la ruta..."
        $testOutPutPath = Join-Path -Path $outputPath -ChildPath "test.txt"
        New-Item -ItemType File -Path $testOutPutPath -Force | Out-Null
        Remove-Item -Path $testOutPutPath -Force
        return $true
    }
    catch {
        write-host "No se tienen permisos de escritura en la ruta especificada."
        return $false
    }    
  }
...
function Write-log {
    param(
        [Parameter(Mandatory = $true)][ValidateSet("INFO", "WARN", "ERROR")]
        [string]$Level,
...
Test-WriteAccess -quiet
Check-IfCSVExists -quiet
Write-log ERROR "Error al exportar CSV. Sin permisos de escritura." -quiet